...
class GradebookApp:
    ...
    def on_course_select(self, event):
        ...
        if selected_course:
            # You can handle course selection events here
            pass
    ...
    def get_selected_course(self):
        try:
            return self.course_listbox.get(self.course_listbox.curselection()[0])
        except IndexError:
            return None
    ...
    def open_add_grade_dialog(self):
        ...
        if selected_student and selected_course:
            AddGradeDialog(self.root, self.gradebook, selected_student, selected_course)
        else:
            messagebox.showwarning("Selection Error", "Please select both a student and a course first.")
...
class AddGradeDialog:
    def __init__(self, parent, gradebook, student_name, course_name):
        self.gradebook = gradebook
        self.student_name = student_name
    ...
    def submit(self):
        ...
        if grade:
            try:
                self.gradebook.record_grade(self.student_name, self.course_name, float(grade))
                self.top.destroy()
                messagebox.showinfo("Success", f"Added grade {grade} for {self.course_name}.")
            except ValueError:
...
class ViewGradesDialog:
    def __init__(self, parent, gradebook, student_name):
        ...
        student = self.gradebook.find_student(student_name)
        if student:
            for course_name, course in student.get_courses().items():
                grades_text = ", ".join(map(str, course.grades))
...
class ManageAttendanceDialog:
    def __init__(self, parent, gradebook, student_name):
        ...
        ttk.Label(date_frame, text="Day:").grid(row=0, column=0)
        self.day_entry = ttk.Entry(date_frame, width=5)
        ...
        ttk.Label(date_frame, text="Year:").grid(row=0, column=4)
    ...
    def mark_present(self):
        date = f"{self.year_entry.get()}-{self.month_entry.get().zfill(2)}-{self.day_entry.get().zfill(2)}"
        ...
        self.send_email("Attendance Update", f"Student {self.student_name} was marked present on {date}.", "parent-email@example.com")
    ...
    def mark_absent(self):
        ...
        self.send_email("Attendance Update", f"Student {self.student_name} was marked absent on {date}.", "parent-email@example.com")
